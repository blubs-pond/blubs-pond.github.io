...
import { appendTerminalOutput, appendTerminalSymbol, appendTerminalHTML, escapeRegExp, appendOutput, displayMap, updateUI, frog, updateGameUI } from '../../terminal/js/ui.js';
...
function updateGameState(dt) { // Primarily game time and phase
...
function updateGame() { // This function is called by gameLoop without dt
...
function handleMonsterAction(monster) {
    ...
    // Example: If monster is near player (but not same room) and hostile, it should try to move to player.
        ...
        if (monster.target !== "player") { // If not already targeting player, make it so
    ...
    // Example: Monster is active, not near player, no specific target - might patrol or random move
        ...
        appendOutput(`Monster ${monster.name || 'Unknown Monster'} is active with no specific target. General movement.`);
...
function maybeMoveMonster(monster) {
    ...
    if (monster.target === "player") {
        ...
        if (monster.path && monster.path.length > 0) {
            ...
            appendOutput(`Monster ${monsterId} (targeting player) moves from ${monster.location} to ${nextStep}. Path steps remaining: ${monster.path.length}`);
            ...
            if (monster.location === gameState.playerLocation) {
                ...
                appendOutput(`Monster ${monsterId}'s path to player ended at ${monster.location}, but player is at ${gameState.playerLocation}.`);
                // Monster might need to re-path or lose target.
            }
        } else {
            appendOutput(`Monster ${monsterId} wants to reach player (${gameState.playerLocation}) but has no current path.`);
    ...
    // --- Case 2: Monster is targeting a specific location (room key) ---
        ...
        if (monster.path && monster.path.length > 0) {
            const nextStep = monster.path.shift();
            appendOutput(`Monster ${monsterId} (targeting room ${targetLocationKey}) moves from ${monster.location} to ${nextStep}. Path steps remaining: ${monster.path.length}`);
            monster.location = nextStep;
            ...
            if (monster.location === targetLocationKey) {
                ...
                appendOutput(`Monster ${monsterId}'s path to ${targetLocationKey} ended at ${monster.location} before arrival.`);
    ...
    // --- Case 3: No specific target / Fallback to random movement (or patrolling if implemented) ---
        ...
        if (currentRoomData && currentRoomData.exits && Object.keys(currentRoomData.exits).length > 0) {
            ...
            if (randomDestinationRoomKey && gameState.locations[randomDestinationRoomKey]) {
                appendOutput(`Monster ${monsterId} randomly moving from ${monster.location} via ${randomDirectionKey} to ${randomDestinationRoomKey}.`);
                monster.location = randomDestinationRoomKey;
            } else {
                appendOutput(`Monster ${monsterId} chose a random exit (${randomDirectionKey}) from ${monster.location} that led to an invalid destination.`);
            }
        } else {
            appendOutput(`Monster ${monsterId} cannot make a random move from ${monster.location} (no exits or room data missing).`);
        }
    }
...
function checkSanityEffects() { // Called by updateGame
    // This function can be used for more complex sanity effects later if needed. (Comment from original)
}

function checkWinLoseConditions() { // Called by updateGame
    if (gameState.sanity <= 0) { // Original code refers to gameState.sanity, not gameState.player.stats.sanity here. Preserving.
...
function handleInventory() {
    ...
    if (gameState.playerInventory.length === 0) { // Original used gameState.playerInventory
...
function setPumpSpeed(speed) { // This is a command-like function
    gameState.pump_speed = Math.max(0, Math.min(100, speed));
    appendTerminalOutput(`Pump speed set to ${gameState.pump_speed}%.`);
}

function restartCore() { // This is a command-like function
    if (!gameState.core_active) {
...
function fixReactorComponent(component) { // This is a command-like function
...
function rebootReactorComponent(component) { // This is a command-like function
...
function rebootCaPanel(roomCode) { // This is a command-like function
    ...
    appendTerminalOutput(`Attempting to reboot CA panel in ${getUserFriendlyLocationName(roomCode)}... (not fully implemented)`);
}

function overclockReactorComponent(component) { // This is a command-like function
    appendTerminalOutput(`Attempting to overclock the ${component}...`);
    // TODO: Implement overclocking logic (Comment from original)
}

function upgradeReactorComponent(component) { // This is a command-like function
...
function flushVentilation() { // Core logic for flushing
...
function handleFlush(systemName) { // Command handler
...
function rebootCamera(cameraNumber) { // Core logic for rebooting camera
...
function handleCam(cameraNumber) { // Command handler
    rebootCamera(cameraNumber);
}

function handleReboot(component) { // Generic reboot handler (likely needs more specific routing)
...
function checkHideMonster() { // This seems more like a status check than an action command
    if (gameState.isHiding) {
        for (const monster of Object.values(gameState.monsters)) {
            if (monster?.isNearPlayer) { // Checks if player is hiding AND monster is near
...
function checkLookAtShadow(locationViewed) { // This performs an action (sanity drain)
...
function handleHelp() { // Removed appendTerminalOutput from params, assuming it's globally available via import
...
function handleClear() {
    ...
    if (terminalOutput) { // Added a null check for safety
...
function handlePeak() { // "peek" might be more conventional for looking, but sticking to "peak"
...
function handleUpgrade(component) { // This was also a top-level function, now a handler
    ...
    appendTerminalOutput(`Upgrading ${component} (not yet implemented via this handler specifically, use 'upgrade component_name' which calls upgradeReactorComponent)`);
}
...
// ==========================================================================
function displayGameStatus() { // For the stat-bar
...
function showRoomStatus(target) { // target is a roomCode
